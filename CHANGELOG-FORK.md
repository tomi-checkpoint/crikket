...
| `ff82d5c` | `sdks/capture/src/` (runtime + ui), `apps/web/public/sdk/capture.global.js` | **Console-capture session mode.** New **Capture console logs** button in the capture chooser. Clicking it starts an open-ended debugger session (console + click/input steps), closes the chooser so the user can interact with the page (the launcher stays, plus a dock with a 3:00 countdown + Cancel), and records while they reproduce the bug. Re-opening the widget and taking a screenshot finalizes console + steps + screenshot into one report — the screenshot path no longer restarts the session, so the accumulated history is preserved, and re-opening no longer resets it. A 180s timeout (`CONSOLE_SESSION_TIMEOUT_MS`) auto-stops the session if no screenshot is taken. Existing screenshot/video/upload flows unchanged. New files: `ui/capture-widget/sections/console-dock.tsx`, `ui/capture-widget/hooks/use-console-countdown.ts`. E2E-verified: the captured payload carried 5 console logs + the clicks/input as steps through to submit. | Reporters can capture the console output and exact steps that lead to a bug while reproducing it, instead of a screenshot with no surrounding context. |
| `f14d1c9` | `sdks/capture/src/` (runtime + ui), `packages/capture-core/src/debugger/`, `apps/web/public/sdk/capture.global.js` | **Fix: console session finalized with "0 logs" after re-opening the widget.** Re-opening mid-session and re-clicking **Capture console logs** restarted the debugger session and discarded the console + steps captured so far, so reports submitted empty. Three fixes: (1) `startConsoleSession()` is idempotent — it resumes the running session instead of replacing it; (2) re-open uses the runtime session as the source of truth (no mid-session reset) and the chooser shows "Console capture in progress — take a screenshot to finish" instead of re-offering the start button; (3) `finalizeSession()` synchronously drains the page event queue (new `DEBUGGER_DRAIN_GLOBAL` same-realm drain) so the last sub-120ms of batched events aren't lost. Regression test in `sdks/capture/test/console-session.test.ts`. Verified live on app.joinco.co: re-open + screenshot now preserves logs (was 0). | The headline "export bugs with console logs" flow actually attaches console + steps; before the fix a common re-open path produced empty reports (bug `0xwo-6wBQ6X4`). |
| `27082b0` | `packages/bug-reports/`, `packages/api/`, `apps/web/.../(dashboard)/` | **Bug-reports dashboard: filter by capture key + one-click bulk status.** An org-member `captureKey.listLabels` procedure feeds a "Capture Key" filter in the toolbar; "select all visible" + a "Set status" quick-menu allow mass status changes without opening the bulk-edit dialog. (This was the "Uncommitted (working tree)" set; now committed — per-file detail below.) | Triaging feedback per customer site and bulk-resolving reports was previously one report at a time. |
| `4a69b78` | `packages/billing/test/` (removed) | **Dropped the stale `packages/billing` test suite.** Polar billing isn't self-hosted (the feature is unused) and the suite had 7 pre-existing failures, including a hard import error (`extractWebhookOccurredAt` missing from `polar-payload.ts`), which blocked the repo-wide `bun test` pre-commit gate. Billing source is untouched (still imported by auth/api/bug-reports and typechecks clean). | `bun test` is green again, so the husky pre-commit hook no longer fails on unrelated billing tests. |

## Dashboard capture-key filter + bulk status (`27082b0`)

The bug-reports dashboard adds two features on top of upstream, shipped in `27082b0`. Per-file detail:
