...
export function submitCapturedReport(input: {
  ...
  return submitTransport({
    ...
    report: {
      ...
      stepsToReproduce: input.draft.stepsToReproduce?.trim() ?? "",
      expectedBehavior: input.draft.expectedBehavior?.trim() ?? "",
      actualBehavior: input.draft.actualBehavior?.trim() ?? "",
      surface: input.draft.surface ?? "unknown",
